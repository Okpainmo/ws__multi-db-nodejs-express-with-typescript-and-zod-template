...
import type { ExtendedWebSocket, WebSocketMessage } from '../types.js';
...
export function initializeWebSocketServer(httpServer: HTTPServer) {
  const wss = createWebSocketServer(httpServer, {
    ...
    onConnection: async (_ws: ExtendedWebSocket, request) => {
    ...
    onMessage: async (ws: ExtendedWebSocket, message: WebSocketMessage) => {
    ...
    onClose: async (ws: ExtendedWebSocket, code, reason) => {
    ...
    // Handle errors
    onError: async (ws: ExtendedWebSocket, error) => {
...
function handleAuthentication(ws: ExtendedWebSocket, payload: unknown) {
  try {
    // Validate and extract userId from payload
    // const { token } = payload as { token: string };
    // const userId = verifyTokenAndGetUserId(token);

    const userId = (payload as { userId: string }).userId; // Simplified example
...
function handleChatMessage(ws: ExtendedWebSocket, payload: unknown) {
  try {
    const { recipientId, message } = payload as { recipientId: string; message: string };
...
export async function gracefulShutdown(wss: any) {
...
export function broadcastSystemNotification(message: string) {
  broadcastMessage({
...
export function sendUserNotification(userId: string, notification: unknown) {
